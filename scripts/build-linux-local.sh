#!/usr/bin/env bash
# Build a portable Linux binary locally with the version derived from the latest
...
#
# Usage:
#   scripts/build-linux-local.sh                # uses latest v* tag
#   scripts/build-linux-local.sh v0.7.0-beta    # uses a specific tag
...
# System packages required for a native Linux Tauri build (matches the
# Linux dep list in .github/workflows/build-release.yml plus the libgtk
# and appindicator headers needed for local dev/build).
for pkg in libwebkit2gtk-4.1-dev libgtk-3-dev libayatana-appindicator3-dev librsvg2-dev patchelf pkg-config build-essential; do
    if ! dpkg-query -W -f='${Status}' "$pkg" 2>/dev/null | grep -q 'install ok installed'; then
        missing_apt+=("$pkg")
    fi
done

total_missing=$(( ${#missing_apt[@]} + ${#missing_cargo[@]} ))

if [ "$total_missing" -gt 0 ]; then
    echo ""
    echo "The following components are required to compile the Linux binary"
...
# (e.g. 0.7.2-beta-37c3484). CI builds skip this — they build straight from
...
(cd src-tauri && cargo tauri build --no-bundle --ci)

# target-dir is overridden to "../target" in src-tauri/.cargo/config.toml,
# so the output sits at the repo root, not under src-tauri/.
BIN="target/release/coinman-portfolio"
if [ -f "$BIN" ]; then
    echo ""
    echo "✓ Built: $(realpath "$BIN")"
    echo "  Version: $VERSION"
    echo ""
    echo "  Note: to run on another Ubuntu machine, libwebkit2gtk-4.1-0 must be"
    echo "        installed on the target system."
fi
